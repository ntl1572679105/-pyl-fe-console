      ...
      >
        <template #bodyCell="{ column, record, index, text }">
          ...
          <template v-if="column.key === 'status'">
            {{ textOrderStatus(text,record.supplierId) }}
          </template>
...
const textOrderStatus = computed(() => {
  ...
  return (value,supplierId) => {
    if(value ===1 && !supplierId){
      return '待分配'
    }else if(value ===1 && supplierId){
      return '配送中'
    }else if(value===2){
      return '到达前置仓'
    }
    // return ['', '待分配', '配送中'][value];
  };
});
...
const getList = (formData) => {
  ...
  if(formData?.status==1){
    ...
    requestParameters.assign = 0
  }else if(formData?.status==2){
    requestParameters.status = 1
    requestParameters.assign = 1
  }else if(formData?.status==3){
    requestParameters.status = 2
  }
  shippingList(requestParameters)
    .then((res) => {
...
const handleDetail = (record) => {
  proxy.$router.push({ path: `/cp-shipping-detail/${record.id}` });
};
