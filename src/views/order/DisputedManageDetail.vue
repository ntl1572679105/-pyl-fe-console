<template>
  <PageContainer>
    ...
    <ApprovalModal ref="approval" @finish="ApprovalModalFinsh"></ApprovalModal>
    <CommonNoteModal ref="commonNoteModal"></CommonNoteModal>
    <CommonCloseModal ref="commonCloseModal" @finish="ApprovalModalFinsh"></CommonCloseModal>
...
import { disputeDetail, disputeRefund } from '@/api/dispute.js';
...
const statusName = computed(() => {
  return (value) => {
    ...
    return ['待审核', '已审核/待用户发货', '售后已收货', '确认退款', '完成'][value];
  };
});
const columns = [
  {
  ...
  disputeDetail(id)
    .then((res) => {
      ...
      orderData.value.push(
        ...
        {
          name: '物流单号',
          value: res.order.deliveryNo || '未发货',
        },
        {
          name: '运费',
