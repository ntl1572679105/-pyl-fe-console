  ...
  <PageContainer>
    <a-card :bordered="false">
      ...
      <div class="table-operator">
        <a-button type="primary" @click="handleAdd" v-permission="proxy.$route.meta.type==1?['operator:jcBanner:add']:['operator:gcBanner:add']" > 新建banner </a-button>
      </div>
      ...
      >
        <template #bodyCell="{ column, record, index, text }">
          <template v-if="column.key === 'action'">
            <a-space>
              <a @click="handleEdit(record)" v-permission="proxy.$route.meta.type == 1 ? ['operator:jcBanner:edit'] : ['operator:gcBanner:edit']">编辑</a>
              <a @click="deleteSub(record)" style="color: #ff4d4f" v-permission="proxy.$route.meta.type == 1 ? ['operator:jcBanner:remove'] : ['operator:gcBanner:remove']">删除</a>
            </a-space>
          </template>
...
import { bannerList, bannerEdit, bannerDelete } from '@/api/miniapp.js';
...
const getList = (formData) => {
  ...
  bannerList(  apiType.value,requestParameters)
    .then((res) => {
...
const changeStatus = (checked, record) => {
  record.loading = true;
  bannerEdit(  apiType.value,{ ...record, status: checked ? 1 : 0 })
    .then(() => {
...
const deleteSub = (record) => {
  proxy.$confirm({
    ...
    cancelText: '取消',
    onOk: () => {
      bannerDelete(apiType.value,record.id).then(() => {
