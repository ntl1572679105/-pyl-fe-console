...
import { recommenderList, recommenderUpdate, recommenderDelete } from '@/api/pb2b/miniapp.js';
...
const getList = (formData) => {
  ...
  requestParameters.type = proxy.type;
  // console.log('loadData request parameters:', requestParameters);
  recommenderList(requestParameters)
    .then((res) => {
...
// 新增、修改
const handleAdd = () => {
  proxy.$refs.recommenderModal.show({ title: '新增', type: props.type });
};
const handleEdit = (record) => {
  proxy.$refs.recommenderModal.show({ title: '编辑', ...record, type: props.type });
};
const modelOk = () => {
  clearForm();
};
