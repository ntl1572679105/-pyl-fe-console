<template>
  ...
  >
    ...
    >
      ...
      <a-form-item label="店铺名称" name="shopId">
        <a-select
...
export default {
  setup(props, { emit }) {
    ...
    const rules = {
      shopId: [{ required: true, message: '请选择' }],
    };
    ...
    const show = (item) => {
      ...
      if (item.id) {
        productState.data = [{ label: item.name, value: item.shopId }];
      } else {
        productState.data = [];
      }
    };
    ...
    const changeProduct = (value, option) => {
      ...
      formState.logo = option.logo;
    };

    watch(formState.shopId, () => {
      productState.data = [];
      productState.fetching = false;
    });
    // 按钮操作
