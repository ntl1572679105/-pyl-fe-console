<template>
  ...
  >
    ...
    >
      ...
      <a-form-item label="类目" name="categoryId" style="display: none">
...
export default {
  setup(props, { emit }) {
    ...
    const show = (item) => {
      ...
      if (item.id) {
        productState.data = [{ label: item.productNo, value: item.productId }];
      } else {
        productState.data = [];
      }
    };
    ...
    const changeProduct = (value, option) => {
      ...
      formState.categoryId = option.categoryIds ? option.categoryIds[0] : undefined;
    };
