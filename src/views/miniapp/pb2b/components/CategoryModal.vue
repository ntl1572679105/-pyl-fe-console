<template>
  ...
  >
    ...
    >
      ...
      <a-form-item label="类目名称" name="categoryId">
        <a-select
      ...
      <a-form-item label="商品编号" name="productId">
        <a-select
          ...
          placeholder="请选择"
          :filter-option="false"
          :not-found-content="productState.listing ? undefined : null"
          :options="productState.list"
          @search="fetchProduct"
          @change="changeProduct"
        >
          <template v-if="productState.listing" #notFoundContent>
            <a-spin size="small" />
          </template>
        </a-select>
      </a-form-item>
      ...
      <a-form-item label="商品主图" name="pcMainImgUrl">
        <a-image :src="formState.pcMainImgUrl" />
      </a-form-item>
    </a-form>
  </a-modal>
</template>
...
<script>
import { recommenderUpdate, recommenderAdd } from '@/api/pb2b/miniapp.js';
import { categoryList, goodsList } from '@/api/pb2b/goods.js';
...
export default {
  setup(props, { emit }) {
    ...
    const show = (item) => {
      ...
      if (item.id) {
        productState.data = [{ label: item.categoryName, value: item.categoryId }];
        productState.list = [{ label: item.productNo, value: item.productId }];
      } else {
        productState.data = [];
        productState.list = [];
      }
    ...
    const fetchCategory = debounce((value) => {
      console.log('fetching category: ', value);
      lastFetchId += 1;
      const fetchId = lastFetchId;
      productState.data = [];
      productState.fetching = true;
      categoryList({ categoryName: value }).then((res) => {
        if (fetchId !== lastFetchId) return;
        productState.data = res.map((item) => ({
          label: item.name,
          value: item.id,
        }));
    ...
    const changeCategory = () => {
      formState.productId = [];
      formState.name = '';
      formState.pcMainImgUrl = '';
    };

    const fetchProduct = debounce((value) => {
      console.log('listing product: ', value);
      lastFetchId += 1;
      const fetchId = lastFetchId;
      productState.list = [];
      ...
      goodsList({
        productNo: value,
        primaryCategoryId: formState.categoryId,
        pageNum: 1,
        pageSize: 20,
      }).then((res) => {
        if (fetchId !== lastFetchId) return;
        productState.list = res.data.map((item) => ({
          label: item.productNo,
          value: item.id,
          name: item.name,
          pcMainImgUrl: item.pcMainImgUrl,
        }));
    ...
    const changeProduct = (value, option) => {
      ...
      formState.pcMainImgUrl = option.pcMainImgUrl;
    };
    ...
    const handleOk = () => {
      formRef.value.validateFields().then((values) => {
        loading.value = true;
        const api = values.id ? recommenderUpdate : recommenderAdd;
        const content = values.id ? '修改成功' : '更新成功';
        api(values)
          .then(() => {
