  ...
  <div>
    ...
    >
      ...
      >
        ...
        <a-form-item label="类型" name="channelType" style="display: none">
          <a-input v-model:value="formState.channelType" disable />
        </a-form-item>
        ...
        <a-form-item v-if="formState.linkType === 1" label="外部链接" name="link">
          <a-input v-model:value="formState.link" placeholder="请输入" :maxlength="500" />
        </a-form-item>
        ...
        <a-form-item label="启用状态" name="status">
          <a-switch v-model:checked="formState.status" :checkedValue="0" default :unCheckedValue="1" />
        </a-form-item>
...
import { bannerAdd, bannerUpdate } from '@/api/pb2b/miniapp.js';
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const rules = {
      ...
      // linkType: [{ required: true, message: '请选择跳转链接类型' }],
    ...
    const show = (item) => {
      ...
      if (item.targetId) {
        productState.data = [{ label: item.productNo, value: item.targetId }];
      } else {
        productState.data = [];
      }
    ...
    const handleOk = () => {
      formRef.value.validateFields().then((values) => {
        loading.value = true;
        const api = values.id ? bannerUpdate : bannerAdd;
