<template>
  <PageContainer>
    <a-card :bordered="false">
      ...
      <ShopModal ref="shopModal" @finish="modelOk" />
    </a-card>
  </PageContainer>
</template>
...
import { listShop, recommenderUpdate, recommenderDelete } from '@/api/pb2b/miniapp.js';
...
const getList = (formData) => {
  ...
  requestParameters.type = 7;
  // console.log('loadData request parameters:', requestParameters);
  listShop(requestParameters)
    .then((res) => {
...
// 新增、修改
const handleAdd = () => {
  proxy.$refs.shopModal.show({ title: '新增', type: 7 });
};
const handleEdit = (record) => {
  proxy.$refs.shopModal.show({ title: '编辑', ...record, type: 7 });
};
const modelOk = () => {
  clearForm();
};
