<template>
  ...
  >
    ...
    >
      ...
      <a-form-item label="联系人" linkman="linkman">
        <a-input v-model:value="formState.linkman" placeholder="请输入" />
      </a-form-item>
      <a-form-item label="手机号码" phone="phone">
        <a-input v-model:value="formState.phone" placeholder="请输入" />
      </a-form-item>
      <a-form-item v-if="type === 1" label="密码" password="password" name="password">
...
import { supplierAdd, supplierEdit, supplierDetail } from '@/api/goods';
...
const rules = reactive({
  ...
  type: [{ required: true, message: '请输入供应商类型' }],
  address: [{ required: true, message: '请输入地址' }],
  logo: [{ required: true, message: '请上传供应商logo' }],
...
const handleOk = () => {
  formRef.value.validateFields().then((values) => {
    ...
    const api = formState.id ? supplierEdit : supplierAdd;
    // formState.status===true?formState.status=1:formState.status=0
    // const content = values.id ? '修改成功' : '新增成功';
    const content = type.value == 2 ? '修改成功' : '新增成功';
    api(apiType.value, formState)
      .then(() => {
        proxy.$message.success(content);
        proxy.$emit('finish', { type: formState.type });
        visible.value = false;
      })
      .finally(() => {
        loading.value = false;
