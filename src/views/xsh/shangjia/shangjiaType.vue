        ...
        <template #bodyCell="{ column, record, index, text }">
          <template v-if="column.key === 'action'">
            <a-space>
              ...
              <a @click="handleJie(record)" style="color: orangered">删除</a>
            </a-space>
          </template>
...
import { cateList, cateEdit, cateDelete } from '@/api/o2o/shangjia.js';
...
const handleEdit = (record) => {
  if (record) {
    proxy.$router.push({
      path: `/xsh-shangjiaTypeDetail/${record.id}`,
    });
  }
};
const handleAdd = () => {
...
const statusChange = (record, checked) => {
  record.loading = true;
  cateEdit({ id: record.id, status: record.status })
    .then(() => {
