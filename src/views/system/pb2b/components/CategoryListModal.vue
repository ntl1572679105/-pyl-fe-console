<template>
  ...
  >
    ...
    >
      ...
      <!--  -->
      <a-form-item label="父级类目" name="parentId" v-if="formState.type == 'add'">
        <a-cascader
...
import { categoryAdd, categoryUpdate, categoryDeatil, categoryDelete } from '@/api/pb2b/system.js';
  ...
  setup(props, { emit }) {
    ...
    const handleOk = () => {
      formRef.value.validateFields().then((values) => {
        loading.value = true;
        const api = values.id ? categoryUpdate : categoryAdd;
        const params = { ...values, shopType: 4 };
